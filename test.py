...
class TestResolverIntegration(unittest.TestCase):
    ...
    async def test_udp_timeout_try_next_nameserver(self):
        ...
        with FastForward(loop) as forward:
            ...
            async def get_nameservers_53_and_54(_, __):
                yield (0.5, ('127.0.0.1', 10053))
                yield (0.5, ('127.0.0.1', 10054))

            resolve, _ = Resolver(get_nameservers=get_nameservers_53_and_54)
        ...
        class AioHttpDnsResolver(aiohttp.abc.AbstractResolver):
            ...
            async def resolve(self, host, port=0, family=socket.AF_INET):
                ...
                try:
                    ...
                    raise OSError(0, f'{host} does not exist') from does_not_exist
                except DnsError as resolver_error:
                    raise OSError(0, f'{host} failed to resolve') from resolver_error
...
def patch_open():
    def mock_open(file_name, *_args, **_kwargs):
        lines = \

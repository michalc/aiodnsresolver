...
Asyncio Python DNS resolver. Pure Python, with no dependencies other than the standard library, threads are not used, and all code is in a single module. The nameservers to query are taken from `/etc/resolve.conf`, read once on creation of `Resolver`.
...
- Changes to `/etc/resolve.conf` will _not_ be noticed after creation of the `Resolver`.
